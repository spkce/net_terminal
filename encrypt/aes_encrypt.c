// advanced encryption standard
// author: karl malbrain, malbrain@yahoo.com

#include  "aes_encrypt.h"

#include <stdlib.h>
#include <stdio.h>
#include <string.h>

static unsigned char Sbox[256] = {		// forward s-box
	};

static unsigned char InvSbox[256] = {	// inverse s-box
	};

static unsigned char Xtime2Sbox[256] = {
	};

static unsigned char Xtime3Sbox[256] = {
	 };

static unsigned char Xtime2[256] = {
	}; 

static unsigned char Xtime9[256] = {
	};

static unsigned char XtimeB[256] = {
	}; 		

static unsigned char XtimeD[256] = {
	}; 

static unsigned char XtimeE[256] = {
	}; 
					
static unsigned char Rcon[11] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
									
static void ExpandKey (unsigned char *key, unsigned char *expkey, unsigned int round);
static void  Encrypt (unsigned char state[16], unsigned char *expkey, unsigned int round);
static void MixSubColumns (unsigned char *state);
static void ShiftRows (unsigned char *state);
static void Decrypt (unsigned char *in, unsigned char *expkey, unsigned char *out, unsigned int aesround);
static void AddRoundKey (unsigned *state, unsigned *key);
static void InvMixSubColumns (unsigned char *state);
static void InvShiftRows (unsigned char *state);


// produce Nk bytes for each round
void ExpandKey (unsigned char *key, unsigned char *expkey, unsigned int round)
{
	unsigned char tmp0, tmp1, tmp2, tmp3, tmp4;
	unsigned idx;

	for( idx = 0; idx < 4; idx++ ) 
	{
		expkey[4*idx+0] = key[4 * idx + 0];
		expkey[4*idx+1] = key[4 * idx + 1];
		expkey[4*idx+2] = key[4 * idx + 2];
		expkey[4*idx+3] = key[4 * idx + 3];
	}

	for( idx = 4; idx < 4 * (round + 1); idx++ ) 
	{
		tmp0 = expkey[4*idx - 4];
		tmp1 = expkey[4*idx - 3];
		tmp2 = expkey[4*idx - 2];
		tmp3 = expkey[4*idx - 1];
		if( !(idx % 4) ) 
		{
			tmp4 = tmp3;
			tmp3 = Sbox[tmp0];
			tmp0 = Sbox[tmp1] ^ Rcon[idx/4];
			tmp1 = Sbox[tmp2];
			tmp2 = Sbox[tmp4];
		}

		//  convert from longs to bytes

		expkey[4*idx+0] = expkey[4*idx - 4*4 + 0] ^ tmp0;
		expkey[4*idx+1] = expkey[4*idx - 4*4 + 1] ^ tmp1;
		expkey[4*idx+2] = expkey[4*idx - 4*4 + 2] ^ tmp2;
		expkey[4*idx+3] = expkey[4*idx - 4*4 + 3] ^ tmp3;
	}
}

// #ifdef NEED_DECRYPT
// restores columns in each of 4 rows
// row0 - unchanged, row1- shifted right 1, 
// row2 - shifted right 2 and row3 - shifted right 3
void InvShiftRows (unsigned char *state)
{
	unsigned char tmp;

	// restore row 0
	state[0] = InvSbox[state[0]], state[4] = InvSbox[state[4]];
	state[8] = InvSbox[state[8]], state[12] = InvSbox[state[12]];

	// restore row 1
	tmp = InvSbox[state[13]], state[13] = InvSbox[state[9]];
	state[9] = InvSbox[state[5]], state[5] = InvSbox[state[1]], state[1] = tmp;

	// restore row 2
	tmp = InvSbox[state[2]], state[2] = InvSbox[state[10]], state[10] = tmp;
	tmp = InvSbox[state[6]], state[6] = InvSbox[state[14]], state[14] = tmp;

	// restore row 3
	tmp = InvSbox[state[3]], state[3] = InvSbox[state[7]];
	state[7] = InvSbox[state[11]], state[11] = InvSbox[state[15]], state[15] = tmp;
}


// restore and un-mix each row in a column
void InvMixSubColumns (unsigned char *state)
{
	unsigned char newstate[4 * 4];
	int i;

	// restore column 0
	newstate[0] = XtimeE[state[0]] ^ XtimeB[state[1]] ^ XtimeD[state[2]] ^ Xtime9[state[3]];
	newstate[5] = Xtime9[state[0]] ^ XtimeE[state[1]] ^ XtimeB[state[2]] ^ XtimeD[state[3]];
	newstate[10] = XtimeD[state[0]] ^ Xtime9[state[1]] ^ XtimeE[state[2]] ^ XtimeB[state[3]];
	newstate[15] = XtimeB[state[0]] ^ XtimeD[state[1]] ^ Xtime9[state[2]] ^ XtimeE[state[3]];

	// restore column 1
	newstate[4] = XtimeE[state[4]] ^ XtimeB[state[5]] ^ XtimeD[state[6]] ^ Xtime9[state[7]];
	newstate[9] = Xtime9[state[4]] ^ XtimeE[state[5]] ^ XtimeB[state[6]] ^ XtimeD[state[7]];
	newstate[14] = XtimeD[state[4]] ^ Xtime9[state[5]] ^ XtimeE[state[6]] ^ XtimeB[state[7]];
	newstate[3] = XtimeB[state[4]] ^ XtimeD[state[5]] ^ Xtime9[state[6]] ^ XtimeE[state[7]];

	// restore column 2
	newstate[8] = XtimeE[state[8]] ^ XtimeB[state[9]] ^ XtimeD[state[10]] ^ Xtime9[state[11]];
	newstate[13] = Xtime9[state[8]] ^ XtimeE[state[9]] ^ XtimeB[state[10]] ^ XtimeD[state[11]];
	newstate[2]  = XtimeD[state[8]] ^ Xtime9[state[9]] ^ XtimeE[state[10]] ^ XtimeB[state[11]];
	newstate[7]  = XtimeB[state[8]] ^ XtimeD[state[9]] ^ Xtime9[state[10]] ^ XtimeE[state[11]];

	// restore column 3
	newstate[12] = XtimeE[state[12]] ^ XtimeB[state[13]] ^ XtimeD[state[14]] ^ Xtime9[state[15]];
	newstate[1] = Xtime9[state[12]] ^ XtimeE[state[13]] ^ XtimeB[state[14]] ^ XtimeD[state[15]];
	newstate[6] = XtimeD[state[12]] ^ Xtime9[state[13]] ^ XtimeE[state[14]] ^ XtimeB[state[15]];
	newstate[11] = XtimeB[state[12]] ^ XtimeD[state[13]] ^ Xtime9[state[14]] ^ XtimeE[state[15]];

	for( i=0; i < 4 * 4; i++ )
		state[i] = InvSbox[newstate[i]];
}

// encrypt/decrypt columns of the key
// n.b. you can replace this with
//      byte-wise xor if you wish.

void AddRoundKey (unsigned *state, unsigned *key)
{
	int idx;

	for( idx = 0; idx < 4; idx++ )
		state[idx] ^= key[idx];
}

void Decrypt (unsigned char *in, unsigned char *expkey, unsigned char *out, unsigned int aesround)
{
	unsigned idx;
	unsigned int round = aesround;
	unsigned char state[4 * 4];

	for( idx = 0; idx < 4; idx++ ) {
		state[4*idx+0] = *in++;
		state[4*idx+1] = *in++;
		state[4*idx+2] = *in++;
		state[4*idx+3] = *in++;
	}

	AddRoundKey ((unsigned *)state, (unsigned *)expkey + round * 4);

	InvShiftRows(state);

	while( round-- )
	{
		AddRoundKey ((unsigned *)state, (unsigned *)expkey + round * 4);
		if( round )
			InvMixSubColumns (state);
	} 

	for( idx = 0; idx < 4; idx++ ) {
		*out++ = state[4*idx+0];
		*out++ = state[4*idx+1];
		*out++ = state[4*idx+2];
		*out++ = state[4*idx+3];
	}
}
//#endif
// exchanges columns in each of 4 rows
// row0 - unchanged, row1- shifted left 1, 
// row2 - shifted left 2 and row3 - shifted left 3
void ShiftRows (unsigned char *state)
{
    unsigned char tmp;

	// just substitute row 0
	state[ 0] = Sbox[state[0]];
	state[ 4] = Sbox[state[4]];
	state[ 8] = Sbox[state[8]];
	state[12] = Sbox[state[12]];

	// rotate row 1
	tmp = Sbox[state[1]];
	state[1] = Sbox[state[5]];
	state[5] = Sbox[state[9]];
	state[9] = Sbox[state[13]];
	state[13] = tmp;

	// rotate row 2
	tmp = Sbox[state[2]];
	state[2] = Sbox[state[10]];
	state[10] = tmp;
	tmp = Sbox[state[6]];
	state[6] = Sbox[state[14]];
	state[14] = tmp;

	// rotate row 3
	tmp = Sbox[state[15]];
	state[15] = Sbox[state[11]];
	state[11] = Sbox[state[7]];
	state[7] = Sbox[state[3]];
	state[3] = tmp;
}

// recombine and mix each row in a column
void MixSubColumns (unsigned char *state)
{
    unsigned char newstate[16];

	// mixing column 0
	newstate[ 0] = Xtime2Sbox[state[ 0]] ^ Xtime3Sbox[state[ 5]] ^ Sbox[state[10]] ^ Sbox[state[15]];
	newstate[ 1] = Xtime2Sbox[state[ 5]] ^ Xtime3Sbox[state[10]] ^ Sbox[state[15]] ^ Sbox[state[ 0]];
	newstate[ 2] = Xtime2Sbox[state[10]] ^ Xtime3Sbox[state[15]] ^ Sbox[state[ 0]] ^ Sbox[state[ 5]];
	newstate[ 3] = Xtime2Sbox[state[15]] ^ Xtime3Sbox[state[ 0]] ^ Sbox[state[ 5]] ^ Sbox[state[10]];

	// mixing column 1
	newstate[ 4] = Xtime2Sbox[state[ 4]] ^ Xtime3Sbox[state[ 9]] ^ Sbox[state[14]] ^ Sbox[state[ 3]];
	newstate[ 5] = Xtime2Sbox[state[ 9]] ^ Xtime3Sbox[state[14]] ^ Sbox[state[ 3]] ^ Sbox[state[ 4]];
	newstate[ 6] = Xtime2Sbox[state[14]] ^ Xtime3Sbox[state[ 3]] ^ Sbox[state[ 4]] ^ Sbox[state[ 9]];
	newstate[ 7] = Xtime2Sbox[state[ 3]] ^ Xtime3Sbox[state[ 4]] ^ Sbox[state[ 9]] ^ Sbox[state[14]];

	// mixing column 2
	newstate[ 8] = Xtime2Sbox[state[ 8]] ^ Xtime3Sbox[state[13]] ^ Sbox[state[ 2]] ^ Sbox[state[ 7]];
	newstate[ 9] = Xtime2Sbox[state[13]] ^ Xtime3Sbox[state[ 2]] ^ Sbox[state[ 7]] ^ Sbox[state[ 8]];
	newstate[10] = Xtime2Sbox[state[ 2]] ^ Xtime3Sbox[state[ 7]] ^ Sbox[state[ 8]] ^ Sbox[state[13]];
	newstate[11] = Xtime2Sbox[state[ 7]] ^ Xtime3Sbox[state[ 8]] ^ Sbox[state[13]] ^ Sbox[state[ 2]];

	// mixing column 3
	newstate[12] = Xtime2Sbox[state[12]] ^ Xtime3Sbox[state[ 1]] ^ Sbox[state[ 6]] ^ Sbox[state[11]];
	newstate[13] = Xtime2Sbox[state[ 1]] ^ Xtime3Sbox[state[ 6]] ^ Sbox[state[11]] ^ Sbox[state[12]];
	newstate[14] = Xtime2Sbox[state[ 6]] ^ Xtime3Sbox[state[11]] ^ Sbox[state[12]] ^ Sbox[state[ 1]];
	newstate[15] = Xtime2Sbox[state[11]] ^ Xtime3Sbox[state[12]] ^ Sbox[state[ 1]] ^ Sbox[state[ 6]];

	memcpy (state, newstate, sizeof(newstate));
}

void  Encrypt (unsigned char state[16], unsigned char *expkey, unsigned int round)
{
    unsigned int i;

	AddRoundKey ((unsigned int*)state, (unsigned int*)expkey);

	for(i = 1; i < round; i++)
	{
		MixSubColumns(state);
		AddRoundKey((unsigned int *)state, (unsigned int *)expkey + i * 4);
	}
	
	ShiftRows(state);
	AddRoundKey ((unsigned int*)state, (unsigned *)expkey + round * 4);
}






/** @fn	int Inter_Aes128Encrypt(unsigned char *pInput, int iInputMaxLen, int iInputLen, unsigned char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey, unsigned int iRound)
 *  @brief
 *  @param (in)	unsigned char * pInput    输入原文缓冲区  
 *  @param (in)	int iInputMaxLen		  输入原文缓冲区长度
 *  @param (in)	int iInputLen			  输入原文数据长度
 *  @param (in)	unsigned char * pOutput   输出缓冲区
 *  @param (in)	int iOutputMaxLen         输出缓冲区大小
 *  @param (in)	int * iOutputLen          加密后数据长度
 *  @param (in)	unsigned char * pKey      加密密钥
 *  @param (in)	unsigned int iRound       加密轮数，标准是10轮，SDK私有协议中，有用4轮的
 *  @return	int
 */
int Inter_Aes128Encrypt(unsigned char *pInput, int iInputMaxLen, int iInputLen, unsigned char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey, unsigned int iRound)
{
    int iMaxEncryptLen = 16;
    unsigned char expkey[256] = {0};
	int iBlock = 0;
	int iFill = 0;
	int i = 0;
	int iLeft = 0;
	unsigned char sPrivateKey[16]={0x6a,0x78,0xa3,0x61,0x1f,0x6e,0xb5,0x67,0xad,0x7a,0xfe,0x68,0xca,0x6f,0xce,0x64};
    if ((NULL == pInput) || (NULL == pOutput) || (NULL == iOutputLen))
    {
        return -1;
    }
    //如果外部没有传入密钥，默认使用内部的密钥
    if (NULL != pKey)
    {
		//AES128算法，key的长度为16个字节。SDK认为外部传入的key中的16个字节都是有效的
        memset(sPrivateKey, 0, sizeof(sPrivateKey));
        memcpy(sPrivateKey, pKey, 16);
    }
	
	*iOutputLen = 0;
	
	iLeft = iInputLen % iMaxEncryptLen;
	
	if (iLeft == 0)
	{
		iBlock = iInputLen/iMaxEncryptLen;
	}
	else
	{
		iBlock = (iInputLen-iLeft)/iMaxEncryptLen+1;
	}
	
	if (iBlock == 0) //处理当加密数据长度为0时，全部填充为0x00之后加密
	{
		iBlock += 1;
	}
	/*原文填充后的长度*/
	iFill = iBlock * iMaxEncryptLen;
	
	if (iInputMaxLen < iFill)
	{
		/*原文缓冲区空间不足*/
		return -1;
	}
	
	if (iOutputMaxLen < iFill)
	{
		/*加密后报文缓冲区空间不足*/
		return -1;
	}
	
	
	/*用0x00进行填充*/
	if (iFill > iInputLen)
	{
		memset(pInput+iInputLen, 0, iFill - iInputLen);
	}
	
	ExpandKey (sPrivateKey, expkey, iRound);
	
	for (i=0; i<iBlock; i++)
	{
		Encrypt ((unsigned char*)pInput+i*iMaxEncryptLen, expkey, iRound);
		memcpy((unsigned char*)pOutput+i*iMaxEncryptLen,  (unsigned char*)pInput+i*iMaxEncryptLen, iMaxEncryptLen);
	}
	
	
	*iOutputLen = iFill;
	return 0;
}



/** @fn	int Inter_Aes128Decrypt(char *pInput, int iInputLen, char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey, unsigned int iRound)
 *  @brief
 *  @param (in)	char * pInput			密文输入缓冲区  
 *  @param (in)	int iInputLen			密文长度
 *  @param (in)	char * pOutput			解密后数据缓冲区
 *  @param (in)	int iOutputMaxLen		解密后数据缓冲区大小
 *  @param (in)	int * iOutputLen		解密后数据长度
 *  @param (in)	unsigned char * pKey    解密密钥
 *  @param (in)	unsigned int iRound     解密轮数
 *  @return	int
 */
int Inter_Aes128Decrypt(char *pInput, int iInputLen, char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey, unsigned int iRound)
{
    int iMaxEncryptLen = 16;
    int iBlock = 0;
    int i = 0;
    unsigned char expkey[256] = {0};
    unsigned char sPrivateKey[16]={0x6a,0x78,0xa3,0x61,0x1f,0x6e,0xb5,0x67,0xad,0x7a,0xfe,0x68,0xca,0x6f,0xce,0x64};
    if ((NULL == pInput) || (NULL == pOutput) || (NULL == iOutputLen))
    {
    	printf("error0\n");
        return -1;
    }
    //如果外部没有传入密钥，默认使用内部的密钥
    if (NULL != pKey)
    {
		//AES128算法，key的长度为16个字节。SDK认为外部传入的key中的16个字节都是有效的
        memset(sPrivateKey, 0, sizeof(sPrivateKey));
        memcpy(sPrivateKey, pKey, 16);
    }
    
    if (iInputLen%iMaxEncryptLen != 0)
    {
        /*应该是iMaxEncryptLen的倍数*/
		printf("error1\n");
        return -1;
    }
    
    if (iOutputMaxLen < iInputLen)
    {
        /*解密后报文缓冲区空间不足*/
		printf("error2\n");
        return -1;
    }
    
    iBlock = iInputLen/iMaxEncryptLen;
    
	ExpandKey (sPrivateKey, expkey, iRound);
    
    for (i=0; i<iBlock; i++)
    {
        Decrypt ((unsigned char*)pInput+i*iMaxEncryptLen, expkey, (unsigned char*)pOutput+i*iMaxEncryptLen, iRound);
    }
    
    *iOutputLen = iInputLen;
    
    return 0;
}

/************************************************************************
* Name : ENCRYPT_LevelFourEncrypt
* Description :  encode by Aes128.
* In :
*	pInput
*		Type : char * 
*		Description : string for encoding
*	iInputLen
*		Type : int
*		Description : length of encoded string 
*	iInputMaxLen
*		Type : int
*		Description : length of encoded buffer 
    pKey
        Type: int
        Description : encode key, if pKey is NULL, function use default key
* Out :
*	pOutput
*		Type : int
*		Description : string of destination.
*	iOutputLen
*		Type : int
*		Description : length of encoded string ,equal to iInputLen
    iOutputMaxLen
        Type : int
        Description : length of output buffer
    
  * Reference :
* Precondition :
* Postcondition :
* Return : success if 0, failed if -1;
* Remarks : 
************************************************************************/
int ENCRYPT_LevelFourEncrypt(unsigned char *pInput, int iInputMaxLen, int iInputLen, unsigned char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey)
{
	//aes加密轮数，轮数为4，这个是非标准的，SDK报文校验中用到了这个接口
    return Inter_Aes128Encrypt(pInput, iInputMaxLen, iInputLen, pOutput, iOutputMaxLen, iOutputLen, pKey, 4);
}

/************************************************************************
* Name : ENCRYPT_LevelFourDecrypt
* Description :  encode by Aes128
* In :
*	pInput
*		Type : char * 
*		Description : string for encoding
*	iInputLen
*		Type : int
*		Description : length of encoded string 
*	iOutputMaxLen
*		Type : int
*		Description : length of decode buffer
    pKey
        Type: int
        Description : encode key, if pKey is NULL, function use default key
* Out :
*	pOutput
*		Type : int
*		Description : string of destination.
*	iOutputLen
*		Type : int
*		Description : length of encoded string ,equal to iInputLen
* Reference :
* Precondition :
* Postcondition :
* Return : success if 0, failed if -1;
* Remarks : 
************************************************************************/
int ENCRYPT_LevelFourDecrypt(char *pInput, int iInputLen, char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey)
{
   //aes加密轮数，轮数为4，这个是非标准的，SDK报文校验中用到了这个接口
   return Inter_Aes128Decrypt(pInput, iInputLen, pOutput, iOutputMaxLen, iOutputLen, pKey, 4);
}

/** @fn	int __stdcall AES_encrypt(unsigned char *pInput, int iInputMaxLen, int iInputLen, unsigned char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey)
 *  @brief
 *  @param (in)	unsigned char * pInput    
 *  @param (in)	int iInputMaxLen    
 *  @param (in)	int iInputLen    
 *  @param (in)	unsigned char * pOutput    
 *  @param (in)	int iOutputMaxLen    
 *  @param (in)	int * iOutputLen    
 *  @param (in)	unsigned char * pKey    
 *  @return	int __stdcall
 */

int aes_encrypt(unsigned char *pInput, int iInputMaxLen, int iInputLen, unsigned char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey)
{
	//aes加密轮数，轮数为10
    return Inter_Aes128Encrypt(pInput, iInputMaxLen, iInputLen, pOutput, iOutputMaxLen, iOutputLen, pKey, 10);
}


/** @fn	int __stdcall AES_decrypt(char *pInput, int iInputLen, char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey)
 *  @brief
 *  @param (in)	char * pInput			  
 *  @param (in)	int iInputLen    
 *  @param (in)	char * pOutput    
 *  @param (in)	int iOutputMaxLen    
 *  @param (in)	int * iOutputLen    
 *  @param (in)	unsigned char * pKey    
 *  @return	int __stdcall
 */
int aes_decrypt(char *pInput, int iInputLen, char *pOutput, int iOutputMaxLen, int *iOutputLen, unsigned char* pKey)
{
	//aes加密轮数，轮数为10
	return Inter_Aes128Decrypt(pInput, iInputLen, pOutput, iOutputMaxLen, iOutputLen, pKey, 10);
}


// void main ()
// {
// 	int i;
// 	unsigned char key[16] = { 0x02, 0x03, 0x04, 0x00, 0x06, 0x07, 0x01, 0x05, 0x0a, 0x08, 0x09, 0x0c, 0x0d, 0x0e, 0x0f, 0x0b};
// 	unsigned char expkey[4 * 4 * (Nr + 1)];
// 	unsigned char in[16] = { 0x00, 0x0a, 0x01, 0x0d, 0x07, 0x08, 0x02, 0x09, 0x0b, 0x03, 0x04, 0x05, 0x06, 0x0c, 0x0e, 0x0f};
// 	unsigned char org[16], out[16];
// 	
// 	for (i = 0; i < 16; i++)
// 	{
// 		org[i] = in[i];
// 	}
// 	ExpandKey (key, expkey);
// 	Encrypt (in, expkey);
// 	//AES_Encrypt16Bytes_sa(in, Xtime2Sbox, expkey, Xtime3Sbox, Sbox, Nr);
// 	
// 	Decrypt (in, expkey, out);
// 	for (i = 0; i < 16; i++)
// 	{
// 		if (org[i] != out[i])
// 		{
// 			printf("error!\n");
// 		}
// 	}
// }


